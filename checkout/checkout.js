let tienda = {
lat:-0.1807,
lng:-78.4678 // Quito (CAMBIA ESTO si la empresa está en otra ciudad)
}

let mapa =
L.map("map").setView([tienda.lat,tienda.lng],13)
...
let texto =
document.getElementById("direccionBusqueda").value
...
if(distancia <= 5) return 2
if(distancia <= 10) return 3
if(distancia <= 20) return 5
if(distancia <= 40) return 8
...
let mensaje =
`Hola, hice el pedido #%23${pedido.id}%0A`+
`Total: $${pedido.total}%0A`+
`Cliente: ${pedido.cliente}%0A`+
`Dirección: ${pedido.direccion}, ${pedido.ciudad}%0A%0A`+
`Quiero pagar por transferencia`

window.open(
`https://wa.me/5270072002text=${mensaje}`
)

alert("Te redirigimos a WhatsApp para completar el pago")
...
// 🔥 AQUÍ PONES TU LINK REAL DE PAYPHONE
let linkPayphone =
